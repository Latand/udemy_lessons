...
from typing import Type, Generic, Union, TypeVar, Optional, cast, List, Any, \
    Sequence, Callable, AsyncContextManager
...
class Transaction:
    ...
    async def __aexit__(self, exc_type, exc_val, exc_tb) -> None:
        ...
        await greenlet_spawn(
            self._current_txn.sync_transaction.__exit__, exc_type, exc_val, exc_tb
        )

    async def rollback(self) -> None:
        await self._current_txn.rollback()

    async def commit(self) -> None:
        if self._current_txn is None:
            return
        if self._strategy == TransactionStrategy.ONE_PER_REQUEST:
            return

        await self._current_txn.commit()
